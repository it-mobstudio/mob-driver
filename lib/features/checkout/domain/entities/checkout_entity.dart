...
class RazorpayOrderEntity {
  ...
  factory RazorpayOrderEntity.fromMap(Map<String, dynamic> map) {
    final amountRaw = (map['amount'] ?? map['amount_in_paise'] ?? 0);
    final amountDouble = amountRaw is num
        ? amountRaw.toDouble()
    ...
    return RazorpayOrderEntity(
      razorpayOrderId:
          (map['razorpay_id'] ?? map['razorpay_order_id'] ?? map['id'] ?? '')
              .toString(),
      platformOrderId: (map['order_id'] ??
              map['mob_order_id'] ??
              map['platform_order_id'] ??
              '')
          .toString(),
  ...
  factory PlacedOrderProductEntity.fromMap(Map<String, dynamic> map) {
    ...
    return PlacedOrderProductEntity(
      ...
      price: double.tryParse(
            (map['price_after_tax'] ?? product['vendor_selling_price'] ?? '0')
                .toString(),
          ) ??
          0,
...
class PlacedOrderPointsEntity {
  const PlacedOrderPointsEntity(
      {required this.totalPoints, required this.message});
  ...
  factory PlacedSubOrderEntity.fromMap(Map<String, dynamic> map) {
    ...
    return PlacedSubOrderEntity(
      ...
      products: productsRaw
          .whereType<Map>()
          .map((e) =>
              PlacedOrderProductEntity.fromMap(Map<String, dynamic>.from(e)))
          .toList(),
    );
  }
}
  ...
  factory PlacedOrderEntity.fromMap(Map<String, dynamic> map) {
    ...
    return PlacedOrderEntity(
      ...
      deliveryAddress: addressMap != null
          ? PlacedOrderAddressEntity.fromMap(addressMap)
          : null,
      items: itemsRaw
          .whereType<Map>()
          .map((e) =>
      ...
      payments: paymentsRaw
          .whereType<Map>()
          .map((e) =>
              PlacedOrderPaymentEntity.fromMap(Map<String, dynamic>.from(e)))
          .toList(),
      pointsSummary:
          pointsMap != null ? PlacedOrderPointsEntity.fromMap(pointsMap) : null,
      suborders: suborderRaw
          .whereType<Map>()
          .map(
              (e) => PlacedSubOrderEntity.fromMap(Map<String, dynamic>.from(e)))
          .toList(),
    );
  }
}
