...
class _AzureConversationalAIState extends State<AzureConversationalAI>
  ...
  Future<void> _startRecording() async {
    try {
      ...
      final path = '${dir.path}/mob_ai_recording.wav';
  ...
  Future<void> _stopAndProcess() async {
    ...
    try {
      ...
      if (_audioFilePath != null) await _sendToAzureSTT(_audioFilePath!);
    } catch (_) {}
  }
  ...
  Future<void> _sendToAzureSTT(String filePath) async {
    ...
    try {
      ...
      if (res.statusCode == 200 && mounted) {
        ...
        final text =
            (data is Map ? data['DisplayText']?.toString() : null) ?? '';
        if (text.isNotEmpty) {
          setState(() {
            _messages.add(AiChatMessage(text, isUser: true));
            _messages.add(const AiChatMessage(
              'Sure! Here are some matching products for you.',
              isUser: false,
            ));
            _suggestions
              ..clear()
